...
void dibuixa_EscenaGL(char objecte, CColor col_object, bool ref_mat, bool sw_mat[4], 
			...
			int nptsU, CPunt3D PC_u[MAX_PATCH_CORBA], GLfloat pasCS, bool sw_PC, std::vector<GLfloat> anglesRobot)
{
	float altfar = 0;
...
// OBJECTE ROBOT
void robot(std::vector<GLfloat> anglesRobot) {
	
/*
	GLfloat Gz = anglesRobot.Gz, Gx = anglesRobot.Gx;
	GLfloat Rz = anglesRobot.Rz, Rx = anglesRobot.Rx, Ry = anglesRobot.Ry;
	GLfloat Qx = anglesRobot.Qx;
*/
	GLfloat Gz = anglesRobot[0], Gx = anglesRobot[1];
	GLfloat Rz = anglesRobot[2], Rx = anglesRobot[3], Ry = anglesRobot[4];
	GLfloat Qx = anglesRobot[5];
	...
	glPushMatrix();
		...
		glPushMatrix();
			...
			gluCylinder(2.0, 0.0, 12.5, 20.0, 20.0);
		glPopMatrix();
		...
		glPushMatrix();
			...
			glPushMatrix();
				...
				glutSolidCylinder(0.5, 15, 20.0, 20.0);
			glPopMatrix();

			// Pivot rotació pinça
			glTranslatef(0.0, -15.0, 0.0);
			glRotatef(45.0, 1.0, 0.0, 0.0);
			glRotatef(Rz, 0.0, 0.0, 1.0); // Rota Pinça
			glRotatef(Rx, 1.0, 0.0, 0.0); // Rota Pinça
			glRotatef(Ry, 0.0, 1.0, 0.0); // Rota Pinça
			...
			glPushMatrix();
				...
				glRotatef(Qx, 1.0, 0.0, 0.0); // Obrir|Tancar Dits
				glPushMatrix();
					glTranslatef(0.0, 0.0, 1.75);
					glScalef(1.0, 0.5, 4.0);
					glutSolidCube(1.0);
				glPopMatrix();
				glPushMatrix();
					glTranslatef(0.0, -0.75, 3.5);
